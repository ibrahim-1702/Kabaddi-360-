...
The system comprises several principal components. The ``Pipeline Runner'' (\texttt{run\_pipeline.py}) serves as the central orchestrator managing execution flow and output serialization. The ``Pipeline Configuration'' (\texttt{pipeline\_config.py}) provides a centralized dataclass with configurable parameters. The ``Pose Extraction Module'' (\texttt{pose\_extract\_cli.py}) handles video ingestion, detection, estimation, format conversion, and cleaning. The ``Temporal Alignment'' module (\texttt{temporal\_alignment.py}) performs pelvis-based DTW alignment. The ``Error Localization'' module (\texttt{error\_localization.py}) computes frame-wise joint errors. The ``Pose Validation Metrics'' module (\texttt{pose\_validation\_metrics.py}) calculates structural and temporal scores. The ``LLM Feedback System'' (\texttt{llm\_feedback/}) comprises the Context Engine, Prompt Builder, and LLM Client. Finally, the ``AR Renderer'' (\texttt{ar\_pose\_renderer.py}) generates skeletal visualizations.
...
The default execution mode is offline batch processing. Configuration flags control optional features: \texttt{enable\_viz} controls AR visualization generation (enabled by default), \texttt{enable\_tts} controls text-to-speech audio from feedback (disabled by default), and \texttt{verbose} controls logging verbosity.
...
The pose extraction module accepts output from any pose estimation model, provided the output maps to COCO-17 format. The current implementation uses MediaPipe with a 33-to-17 mapping via \texttt{mp33\_to\_coco17.py}, but alternative models such as MoveNet or OpenPose can be integrated by implementing a corresponding format conversion module.
