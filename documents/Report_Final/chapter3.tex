...
Augmented Reality (AR) visualizations are handled inherently within this ecosystem. Rather than leaving students with raw numbers, the application renders a ``ghost'' skeletal overlay mapping the expert's ideal posture directly atop the recorded video. Visual markers differentiate between the expert trace and the user's execution, delivering immediate spatial corrections.
...
The \textbf{Tutorial} model stores expert technique definitions. Each tutorial record contains a unique identifier, a technique name (e.g., ``hand\_touch'', ``bonus''), a description, and the file path to the pre-extracted expert pose array stored as a NumPy file of shape $(T, 17, 2)$.
