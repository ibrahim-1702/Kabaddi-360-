...
def aggregate_joint_stats(errors: np.ndarray) -> Dict[str, Dict[str, float]]:
    ...
    for joint_idx in range(17):
        ...
        # Use nan-aware functions to handle missing joints
...
def export_frame_joint_errors(errors: np.ndarray) -> Dict[int, Dict[str, float]]:
    ...
    This function enables temporal visualization of error patterns by providing
    per-frame per-joint error values.
    
    Args:
        errors: Frame-wise, joint-wise errors (T, 17)
    
    Returns:
        Dictionary mapping frame index to joint-wise errors
        
    Example:
        {
            0: {"nose": 0.042, "left_knee": 0.068, ...},
            1: {"nose": 0.045, "left_knee": 0.072, ...},
            ...
        }
    
    Design Note:
        Required for Level-3 Step-2 (visual error localization).
        Enables frame-by-frame color coding without recomputation.
        Each frame gets its own set of joint errors for temporal visualization.
    """
...
def export_json(
    ...
    frame_stats: Dict,
    phase_stats: Dict,
    frame_joint_errors: Dict,  # NEW: frame-wise joint errors for visualization
    expert_path: str,
    user_path: str,
    output_path: str
) -> None:
    ...
    Schema enforces:
        ...
        - Frame-wise joint errors for visualization
    ...
    Args:
        ...
        frame_joint_errors: Frame-wise joint errors (for visualization)
    ...
    output = {
        ...
        "frame_joint_errors": {
            str(k): v for k, v in frame_joint_errors.items()  # Convert int keys to strings for JSON
        }
    }
...
def main():
    ...
    # ========================================================================
    # STEP 6: Export Frame-Wise Joint Errors (for visualization)
    # ========================================================================
    print("\n[STEP 6] Exporting frame-wise joint errors for visualization...")
    
    frame_joint_errors = export_frame_joint_errors(errors)
    
    print(f"  ✓ Frame-wise joint errors exported: {len(frame_joint_errors)} frames")
    print(f"    Each frame contains errors for all 17 joints")
    print(f"    Required for Level-3 Step-2 (visual error localization)")
    
    # ========================================================================
    # STEP 7: Export JSON
    # ========================================================================
    print("\n[STEP 7] Exporting structured JSON...")
    ...
    print(f"    - {len(frame_joint_errors)} frames in 'frame_joint_errors' (NEW)")
    ...
    print(f"  - Frame-joint errors: per-frame per-joint errors (for visualization)")
