...
def main():
    ...
    if len(sys.argv) < 2:
        print(
            "Usage: python -m llm_feedback.context_engine <results.json> [output.json]")
        print("\nExample:")
        print(
            "  python -m llm_feedback.context_engine data/results/session_id/results.json")
        sys.exit(1)
    ...
    try:
        ...
        print(
            f"  ✓ Frames: {raw_scores['error_statistics']['metadata']['num_frames']}")

        # Generate context
        print(f"\n[2/3] Generating canonical context...")
        context = generate_context(raw_scores)
        print(
            f"  ✓ Overall: {context['summary']['overall_score']}% ({context['summary']['overall_assessment']})")
        print(
            f"  ✓ Major deviations: {len(context['joint_deviations']['major'])}")
